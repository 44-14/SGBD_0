...
//try {
		...
		log.debug("creation..." + t.toString());
		 em.persist(t);
		 em.flush();
		em.getTransaction().commit();
		 //raccrochage entre les données et la DB
		t=  em.merge(t);
		log.debug("Merging..."+ t.toString());
...
*/
	/* @Override
	 public AbstractActive deleteLogically(AbstractActive t) {
	 	log.info(t.toString());
		 EntityManager em = null;
		 try {
			 em = EMF.getEM();
	...
	public T findOne(T t, int id) {
		...
		try {
			...
			log.debug("c'est vraiment mais vraiment raté ");
		} finally {
	        em.close();
	        log.debug("Close em : Ok");
	    }
